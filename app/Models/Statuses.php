...
class Statuses extends Model
{
    ...
    public function getStatuses($user_id)
    {
        ...
        $statuses = DB::select("
        SELECT users.name, users.profile_image, statuses.`status`,statuses.has_attachment,statuses.attachments,statuses.user_id,statuses.status_id,statuses.created_at,is_users_tagged,
...
WHERE statuses.user_id = $user_id ORDER BY statuses.status_id DESC", [1]);
    ...
    public function discoverStatuses($user_id)
    {
        ...
        // ->select('users.name','is_users_tagged','statuslikes.user_id','profile_image','statuses.has_attachment','statuses.attachments','status','statuses.user_id','statuses.status_id','statuses.created_at',DB::raw("avg(ratting) as ratting"),
        // DB::raw("count(statuslikes.id) as likes_count"), DB::raw("count(status_shares.id) as shares_count"), DB::raw("count(status_comments.id) as comments_count"),
        ...
        RIGHT JOIN statuses on statuses.user_id = followers.`followed_user_id`
        LEFT JOIN users on users.user_id = statuses.`user_id`
        WHERE followers.follower_user_id = $user_id ORDER BY statuses.status_id DESC
    ...
    public function getStatusesForReact($user_id)
    {
    ...
    public static function getStatusForReactWeb($user_id, $status_id)
    {
    ...
    FROM statuses
    LEFT JOIN users on users.user_id = statuses.user_id
    WHERE statuses.status_id = '$status_id'", [1]);
        return $statuses;
    }
}
