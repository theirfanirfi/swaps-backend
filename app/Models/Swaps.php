...
LEFT JOIN status_shares as sharestatus on sharestatus.status_id = swaps.status_id AND sharestatus.user_id = swaps.swaped_with_user_id
...
WHERE `swaped_with_user_id` = '$user_id' and is_expired = '1' and is_reviewed = '1'
    ...
    public function getExpiredSwapsForReviewIfNotReviewed($user_id)
    {
        //$user_id = 3;
        date_default_timezone_set('Asia/Karachi');
        $time = new DateTime("now");
    ...
    where (poster_user_id = '$user_id' and time_to_sec(timediff('$time',time_of_swap))/60 > 1440 and is_accepted = 1 and is_reviewed = 0) or (swaped_with_user_id = '$user_id' and time_to_sec(timediff('$time',time_of_swap))/60 > 1440 and is_accepted = 1 and is_reviewed = 0) ORDER BY swap_id DESC LIMIT 20;
    ...
    public static function getPosterByStatusId($status_id)
    {
        return Statuses::where(['status_id' => $status_id])->first()->user_id;
    }
}
