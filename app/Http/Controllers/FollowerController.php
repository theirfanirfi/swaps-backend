...
class FollowerController extends Controller
{
    ...
    public function swapStatus(Request $req)
    {
        ...
        $swapon = $req->input('on');
        $swaped_with_user_id = $req->input('id');
        $verify = new VerifyToken();
        $user = $verify->verifyTokenInDb($token);
        ...
        if (!$user) {
        ...
        } else if ($token == "" || $start_date == "" || $status_id == "" || $enddate == "" || $swaped_with_user_id == "") {
            return response()->json([
                'isAuthenticated' => true,
                'isSwaped' => false,
                'isAlready' => false,
            ...
            $poster_id = Swaps::getPosterByStatusId($status_id);
            ...
            $swap = Swaps::where(['swaped_with_user_id' => $swaped_with_user_id, 'status_id' => $status_id, 'swaper_id' => $user->user_id]);
            if ($swap->count() > 0) {
                return response()->json([
                    'isAuthenticated' => true,
                    'isSwaped' => false,
                ...
                $sw = new Swaps();
                $sw->poster_user_id = $poster_id;
                $sw->swaped_with_user_id = $swaped_with_user_id;
                $sw->status_id = $status_id;
                $sw->swap_start_date = $start_date;
                $sw->swap_end_date = $enddate;
                $sw->swap_on = $swapon;
                $sw->swaper_id = $user->user_id;
    ...
    public function getFollowedUsersForSwap(Request $req)
    {
        $token = $req->input('token');
        if ($token == null) {
            return response()->json([
            ...
            if (!$user) {
                ...
                $followed = Followers::getFollowedUserForReact($user->user_id);
                if ($followed->count() > 0) {
                    return response()->json([
                        'isAuthenticated' => true,
                        'isError' => false,
                    ...
                    return response()->json([
                        ...
                        'isEmpty' => true,
                        'isFound' => false,
                        'message' => 'You have not followed any user to swap status with. Follow users first.'
                    ]);
                }
            }
        }
