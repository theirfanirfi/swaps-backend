...
class AttachmentController extends Controller
{
    ...
    /////////////// group chat audio messsage


    public function sendGroupAudio(Request $req){

        $token = $req->input('token');
        $CHAT_ID = $req->input('id');
...
$isPartOfTheGroup = GroupMessages::checkUserInGroupWhetherPartOrNot($user->user_id,$CHAT_ID);
    ...
    if($req->hasFile('audio')){
        //upload image
            ...
            if($file->move($path,$file_name)){
                ...
                $msg->sender_id = $user->user_id;
                $msg->reciever_id = 0;
                $msg->chat_id = $isPartOfTheGroup->first()->chat_id;
                $msg->is_audio = 1;
                $msg->audio_attachment = $file_name;
